...
def getPositions(data):
    ...
    triad = np.asarray([[-2.25, 2.25, 0], [2.25, 2.25, 0], [0, -2.3, 0]])  # sensor locations

    manta = triad + (arrays[0] % 4) * np.asarray([0, -19.5, 0]) + (arrays[0] // 4) * np.asarray([19.5, 0, 0])

    for array in range(1, len(arrays)): #Compute sensor positions -- probably not necessary to do each call
        manta = np.append(manta, (triad + (arrays[array] % 4) * np.asarray([0, -19.5, 0]) + (arrays[array] // 4) * np.asarray([19.5, 0, 0])), axis=0)
...
if  input("Regenerate Fields?"):
    ...
    start = 0
    fin = 200
    Fields_baseline = processData("")[:, :, :, start:fin]
